...
def average(one_list):
    ...
    for number in one_list:
        total_sum_of_list += number
    mean= total_sum_of_list/len(one_list)
    return mean
...
def lowest_squared(one_list):
    list_squared =  [number*number for number in one_list]
    return min(list_squared)
...
def highest_squared(one_list):
    list_squared =  [number*number for number in one_list]
    return max(list_squared)

    #list_squared = map(lambda number: number*number, one_list)
...
def starts_with_a(one_list):
    #return [element for element in one_list if element[0]=="a"]
    #return map(lambda element: element[0]=="a", one_list)
    return list(filter(lambda element: element[0]=="a", one_list))
...
def reverse_each_element(one_list):
    #return list(filter(lambda element: element[::-1], one_list))  #filter is sieving, mapping from one element to another
        #will always evaluate to true, as it's not empty
    #return map(lambda element: ) #would need to use a function using a map, apply same function to each element
#???????? WHY DOESN'T WORK????
    return [element[::-1] for element in one_list]
...
#order based on last letter of each word
#identify the last letter of each word 
#and then order it in reverse_each_element
#how do I make sure I keep what it belongs to?

def sort_by_last_letter(one_list):
    return sorted(one_list, key=lambda element: element[-1]) 
...
def greater_than_5(one_list):
    return list(filter(lambda element: element > 5, one_list))
...
def greater_than(one_list, one_number):
    return list(filter(lambda element: element > one_number, one_list))
...
def less_than(one_list, one_number):
    #return filter(lambda element: element if element < one_number, one_list) ##WHY DOES THIS NOT WORK?????
    #element if is implicit
    return list(filter(lambda element: element < one_number, one_list))
    #return [element for element in one_list if element < one_number]
...
def words_shorter_than(one_list, one_number):
    return list(filter(lambda element: len(element) < one_number, one_list))

#list = 'banana', 'apple', 'orange', 'nut', 'avocado'

#print(words_shorter_than(list, 6))
...
#>>> [c in "12345678!" for c in "!$%&@"]
#[True, False, False, False, False]

#>>> any(c in "12345678!" for c in "!$%&@")    # are "any" of these values True?
#True


def all_above(one_list, one_number):
    return all(element >one_number for element in one_list)
...
def all_below(one_list, one_number):
    return all(element <one_number for element in one_list)
...
def multiply_each_by(one_list, one_number):
    return [element * one_number for element in one_list]
...
def add_key_value_pair(dictionary, key, value):
    #updated_dictionary = dictionary.update(key, value)
...
def where_value_below(dictionary, number):
    #return dict(filter(lambda animal: animal[1] < number, dictionary.items()))
...
def where_key_starts_with(dictionary, letter):
    return {key: value for (key, value) in dictionary.items() if key[0] == letter}
